...
describe("access tokens", () => {
  it("round-trips sub/role/sessionId through sign + verify", async () => {
    const token = await signAccessToken({ sub: "user_1", role: "ADMIN", sessionId: "sess_1" });
  ...
  it("rejects a token signed with a different secret", async () => {
    ...
    const token = await signAccessToken({ sub: "user_1", sessionId: "sess_1" });
